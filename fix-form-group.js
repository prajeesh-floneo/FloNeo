...
async function fixFormGroup() {
  try {
    ...
    if (!app || !app.canvas) {
      console.error("❌ App 5 or canvas not found");
      return;
    }

    console.log("📄 Found canvas for app 5");

    // Parse the canvas state
    const canvasState = JSON.parse(app.canvas.canvasState);
    ...
    const existingFormGroup = firstPage.groups.find(
      (g) => g.id === "form-group-1760870767077"
    );
    ...
    const submitButton = firstPage.elements.find(
      (el) =>
    ...
    const verifyCanvasState = JSON.parse(verifyApp.canvas.canvasState);
    const verifyFormGroup = verifyCanvasState.pages[0].groups?.find(
      (g) => g.id === "form-group-1760870767077"
    );
    ...
    if (verifyFormGroup) {
      ...
      console.error(
        "❌ Verification failed - form group not found in database"
      );
    }
  } catch (error) {
