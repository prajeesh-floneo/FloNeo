...
export async function GET(
  ...
  try {
    ...
    const targetUrl = `${BACKEND_URL}/api/media/thumbnails/${encoded}`;
    return await proxyMediaAsset(request, targetUrl);
  } catch (error) {
    console.error("❌ Media thumbnail proxy error:", error);
    return NextResponse.json(
      { success: false, message: "Failed to fetch media thumbnail" },
      { status: 502 }
    );
  }
}
