...
export const ChartElement: React.FC<ChartElementProps> = ({
  ...
  const resolvedValueKey = mergedProps.valueKey || mergedProps.yKey;
  const isBindableChart =
    normalizedType === "chart-bar" ||
    normalizedType === "chart-line" ||
    normalizedType === "chart-pie" ||
    normalizedType === "chart-donut";

  const toAppIdString = (value: unknown): string | null => {
  ...
  const resolvedAppId =
    toAppIdString(mergedProps.appId) ||
    toAppIdString((properties as any)?.appId) ||
    toAppIdString((properties as any)?.appID) ||
    toAppIdString((properties as any)?.app_id) ||
    toAppIdString(currentAppId);

  const canFetchChartData =
    Boolean(resolvedAppId && mergedProps.tableName && mergedProps.xKey && resolvedValueKey) &&
    isBindableChart;

  const [remoteDataset, setRemoteDataset] = useState<Array<Record<string, any>> | null>(null);
  ...
  const chartData = canFetchChartData ? remoteDataset ?? [] : data;
  const shouldShowLoadingState = canFetchChartData && fetchStatus === "loading" && remoteDataset === null;
  const shouldShowErrorState = canFetchChartData && fetchStatus === "error";
  ...
  useEffect(() => {
    const tableName = mergedProps.tableName;
    const xAxisKey = mergedProps.xKey;
    const yAxisKey = resolvedValueKey;

    if (!canFetchChartData || !resolvedAppId || !tableName || !xAxisKey || !yAxisKey) {
    ...
    const loadChartData = async () => {
      ...
      try {
        const params = new URLSearchParams({
          ...
          limit: String(mergedProps.fetchLimit || 500),
          _t: Date.now().toString(),
        });

        const response = await authenticatedFetch(
          `/api/chart-data/${resolvedAppId}/tables/${encodeURIComponent(tableName)}?${params.toString()}`,
          { signal: controller.signal }
        );
        const payload = await response.json();

        if (!response.ok || payload.success === false) {
          throw new Error(payload.message || "Failed to fetch chart data");
        }

        if (cancelled) return;

        setRemoteDataset(Array.isArray(payload.data) ? payload.data : []);
        setFetchWarnings(Array.isArray(payload.warnings) ? payload.warnings : []);
        ...
        setFetchError(error instanceof Error ? error.message : "Failed to load chart data");
      }
    };
    ...
    canFetchChartData,
    mergedProps.fetchLimit,
    mergedProps.tableName,
    mergedProps.xKey,
    normalizedType,
    resolvedAppId,
    resolvedValueKey,
  ...
  const renderBarChart = () => {
    ...
    const fallbackKey = resolvedValueKey || mergedProps.valueKey || "value";
    const config =
      series.length > 0
        ? series
        : [{ dataKey: fallbackKey, label: fallbackKey }];
    const customBarColor = mergedProps.barColor;
    const barSize =
      typeof mergedProps.barWidth === "number" && mergedProps.barWidth > 0
        ? mergedProps.barWidth
        : undefined;
        ...
        >
          ...
          {config.map((serie, index) => (
            <Bar
              ...
              name={serie.label}
              fill={
                serie.color || customBarColor || colors[index % colors.length]
              }
              barSize={barSize}
              radius={[6, 6, 0, 0]}
            />
  ...
  const renderLineChart = () => {
    ...
    const config =
      series.length > 0
        ? series
        : [{ dataKey: fallbackKey, label: fallbackKey }];
    const customLineColor = mergedProps.lineColor;
    const showMarkers = mergedProps.showMarkers !== false;
        ...
        >
          ...
          {config.map((serie, index) => (
            <Line
              ...
              name={serie.label}
              stroke={
                serie.color || customLineColor || colors[index % colors.length]
              }
              strokeWidth={serie.strokeWidth || 3}
              dot={showMarkers ? { r: 4 } : false}
              activeDot={showMarkers ? { r: 6 } : undefined}
            />
          ))}
        </LineChart>
      </ResponsiveContainer>
  ...
  const renderPieChart = () => {
    ...
    const pieData = chartData.filter((entry) => {
      const value = entry?.[valueKey];
      return typeof value === "number" && Number.isFinite(value);
    });
    const totalValue = pieData.reduce((acc, entry) => {
      const numericValue = entry?.[valueKey];
      return acc + (typeof numericValue === "number" ? numericValue : 0);
    }, 0);
      ...
      <ResponsiveContainer width="100%" height="100%">
        <PieChart>
          <Pie
            ...
            paddingAngle={4}
          >
            {pieData.map((entry: Record<string, any>, index: number) => (
              <Cell
              ...
              const payloadPercentage =
                typeof payload?.payload?.percentage === "number"
                  ...
                  : totalValue > 0
                    ...
                    : 0;
              const labelText =
                payload?.payload?.[nameKey] ?? name ?? nameKey ?? "value";
              const formattedValue = Number.isFinite(numericValue)
                ? numericValue.toLocaleString()
                : String(value ?? "0");
              return [`${formattedValue} (${payloadPercentage}%)`, labelText];
            }}
            contentStyle={{
              fontSize: "12px",
              borderRadius: "10px",
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 p-3 overflow-hidden">
        {shouldShowLoadingState ? (
          <div className="flex h-full items-center justify-center rounded-md border border-dashed border-slate-300 text-xs text-slate-500 dark:text-slate-400">
            Loading chart data...
          </div>
        ) : shouldShowErrorState ? (
          <div className="flex h-full items-center justify-center rounded-md border border-dashed border-red-200 text-xs text-red-600 dark:text-red-400 text-center px-4">
            Failed to load chart data{fetchError ? `: ${fetchError}` : ""}
          </div>
        ) : showEmptyState ? (
      ...
      {canFetchChartData && fetchWarnings.length > 0 && (
